...
function getRandomInitTileValue()
    ...
    local random_number = love.math.random(1, 42)
...
function drawMap()
    ...
    for x, p in ipairs(map_matrix) do
        for y, p2 in ipairs(map_matrix[x]) do
            ...
            local new_scale_factor = scale_factor
            ...
            -- Check if the tile needs to be larger.  -- TODO: draw this tile after all other tiles.
            if map_matrix[x][y] >= 60 then
                new_scale_factor = 2
                tile_x = tile_x - tile_size/2
                tile_y = tile_y - tile_size/2
            end

            -- Draw tile at x,y  ('% 10' means that 16 and 6 are considered the same tile, tree_used)
            if map_matrix[x][y] % 10 == 0 then
                love.graphics.draw(grass, tile_x, tile_y, 0, new_scale_factor, new_scale_factor)
            elseif map_matrix[x][y] % 10 == 1 then
                love.graphics.draw(wheat1, tile_x, tile_y, 0, new_scale_factor, new_scale_factor)
            elseif map_matrix[x][y] % 10 == 2 then
                love.graphics.draw(wheat2, tile_x, tile_y, 0, new_scale_factor, new_scale_factor)
            elseif map_matrix[x][y] % 10 == 3 then
                love.graphics.draw(wheat3, tile_x, tile_y, 0, new_scale_factor, new_scale_factor)
            elseif map_matrix[x][y] % 10 == 4 then
                love.graphics.draw(destroyed, tile_x, tile_y, 0, new_scale_factor, new_scale_factor)
            elseif map_matrix[x][y] % 10 == 5 then  -- Draws an apple tree and grass.
                ...
                love.graphics.draw(tree, tile_x, tile_y, 0, new_scale_factor, new_scale_factor)
            elseif map_matrix[x][y] % 10 == 6 then  -- Draws an apple tree and grass.
                love.graphics.draw(grass, tile_x, tile_y, 0, new_scale_factor, new_scale_factor)
                love.graphics.setColor(1, 1, 1, 1)  -- Tree has a white colour mod (normal)
                love.graphics.draw(tree_used, tile_x, tile_y, 0, new_scale_factor, new_scale_factor)
            end

            -- Check if a fence needs to be drawn over the tile.
            if map_matrix[x][y] >= 30 and map_matrix[x][y] < 60 or map_matrix[x][y] >= 30+60 and map_matrix[x][y] < 60+60 then
                love.graphics.setColor(1, 1, 1, 1)  -- Tree has a white colour mod (normal)
                love.graphics.draw(fence_single, tile_x, tile_y, 0, new_scale_factor, new_scale_factor)
            end
        end
    end
end
